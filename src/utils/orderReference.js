...
const { requireCompanyId, requireActiveCompanyId } = require("./tenantScope");
...
/**
 * Atomically issue the next per-company order_reference.
 * Shared by `orders` and `added_orders` for the same company.
 */
async function allocateNextOrderReference(companyId) {
  const id = requireCompanyId(companyId || requireActiveCompanyId());
  const { data, error } = await supabase.rpc("next_company_order_reference", {
    p_company_id: id,
  });
  ...
  if (!Number.isFinite(issued) || issued <= 0) {
    throw new Error("Failed to allocate company order reference");
  }

  return Math.trunc(issued);
}
